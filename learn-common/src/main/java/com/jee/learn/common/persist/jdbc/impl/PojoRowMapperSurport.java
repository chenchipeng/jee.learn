...
/**
 * 
 * <p>Title: PojoRowMapperSurport</p>
 * <p>Description: Pojo对象转换器，用于将jdbc查询结果转换为pojo对象</p>
 ...
 */
public class PojoRowMapperSurport {

	private Class<?> clazz;

	public PojoRowMapperSurport(Class<?> clazz) {
		super();
		this.clazz = clazz;
	}
		...
		try {
			...
			for (Field fd : fields) {
				...
				if (isExistColumn(rs, columnLabel)) {
					...
					} else if (pojoTypeName.equals("java.util.Set")) {
						// setMthd.invoke(pojo, rs.getString(columnLabel));
					} else {
