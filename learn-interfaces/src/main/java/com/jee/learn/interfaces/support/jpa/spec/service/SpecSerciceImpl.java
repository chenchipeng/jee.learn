package com.jee.learn.interfaces.support.jpa.spec.service;
...
import com.jee.learn.interfaces.support.jpa.spec.Filter;
import com.jee.learn.interfaces.support.jpa.spec.QueryParams;
import com.jee.learn.interfaces.support.jpa.spec.repository.SpecRepository;
...
public class SpecSerciceImpl<T, ID extends Serializable> implements SpecSercice<T, ID> {
	...
	public void logicDelete(T entity) {
		...
		ReflectUtils.invokeSetter(entity, DEL_FLAG_NAME, Y);
		this.save(entity);
	}
	...
	private Object primaryKeyValidation(T entity) {
		Object primaryKey = ReflectUtils.invokeGetter(entity, PRIMARY_KEY_NAME);
		if (primaryKey == null || (primaryKey instanceof String && StringUtils.isBlank(String.valueOf(primaryKey)))) {
			throw new IdentifierGenerationException("ids for this class must be manually assigned before calling primaryKeyValidation()");
		}
		return primaryKey;
	}
