...
import com.jee.learn.manager.support.dao.Sort.Direction;
...
/**
 * Title: ConditionHandler<br/>
 * Description: 查询处理器<br/>
    ...
    protected Predicate[] parseCondition(Root<T> root, CriteriaBuilder cb, Condition con) {
        ...
        for (Condition.Paramerter p : con) {
            ...
            case EQ:
                ...
                break;
            case EQEX:
                Path<?> path2 = getPath(root, (String) p.getValue());
                list.add(cb.equal(path, path2));
                break;
            case NE:
                list.add(cb.notEqual(path, p.getValue()));
                break;
            case NEEX:
                Path<?> path3 = getPath(root, (String) p.getValue());
                list.add(cb.notEqual(path, path3));
                break;
            case LIKE:
